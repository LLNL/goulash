 ...
 */
        ...
        CXXPROTO(rush_larsen_cpu_omp_mpi_compiler1);
        CXXPROTO(rush_larsen_cpu_omp_mpi_compiler2);
        CXXPROTO(rush_larsen_cpu_omp_mpi_compiler3);
        CXXPROTO(rush_larsen_gpu_omp_mpi_compiler1);
        CXXPROTO(rush_larsen_gpu_omp_mpi_compiler2);
        CXXPROTO(rush_larsen_gpu_hip_mpi_compiler1);
        CXXPROTO(rush_larsen_gpu_hip_mpi_compiler2);
        CXXPROTO(rush_larsen_gpu_lambda_hip_mpi_compiler1);
        CXXPROTO(rush_larsen_gpu_lambda_hip_mpi_compiler2);
        FORTPROTO(rush_larsen_cpu_omp_mpi_fort_compiler1);
        FORTPROTO(rush_larsen_gpu_omp_mpi_fort_compiler1);
...
#define FORTLAUNCH(x) if (rank == 0) printf_interop("==== Calling %s()  Iter %i of %i %s====\n", rushxstr( x ),  iter, interop_iterations, INTEROP_COMPILER); fail_count += x##_ (rush_iterations, kernel_mem_used); print_mem_stats("after calling " rushxstr( x) "()", iter, interop_iterations )
    ...
    for (iter=1; iter <= interop_iterations; iter++)
    {
         ...
         */
        CXXLAUNCH(rush_larsen_cpu_omp_mpi_compiler1);
        CXXLAUNCH(rush_larsen_cpu_omp_mpi_compiler2);
        CXXLAUNCH(rush_larsen_cpu_omp_mpi_compiler3);
        CXXLAUNCH(rush_larsen_gpu_omp_mpi_compiler1);
        CXXLAUNCH(rush_larsen_gpu_omp_mpi_compiler2);
        CXXLAUNCH(rush_larsen_gpu_hip_mpi_compiler1);
        CXXLAUNCH(rush_larsen_gpu_hip_mpi_compiler2);
        CXXLAUNCH(rush_larsen_gpu_lambda_hip_mpi_compiler1);
        CXXLAUNCH(rush_larsen_gpu_lambda_hip_mpi_compiler2);
        FORTLAUNCH(rush_larsen_cpu_omp_mpi_fort_compiler1);
        FORTLAUNCH(rush_larsen_gpu_omp_mpi_fort_compiler1);
